...
namespace POC
{
    ...
    public class SampleHandlerWhiteSpaceold : IExternalEventHandler
    {
                ...
                using (Transaction transaction = new Transaction(_doc))
                {
                    ...
                    foreach (KeyValuePair<int, List<ConduitGrid>> kvp in conduitGridDictionary)
                    {
                        ...
                        if (kvp.Value.Any())
                        {
                            ...
                            SubTransactionForTag(_doc, elements, out List<XYZ> minList, out List<XYZ> maxList, out double maxHeight, out double maxWidth);
                            //transaction.Commit();   
                            //return;
                            elements = Utility.ConduitInOrder(elements);
                         ...
                         //  Utility.CreateConduit(_doc, (elements[0] as Conduit), crossPoints.Key, crossPoints.Value);
                            ...
                            double round = Math.Floor((length * 2) / maxHeight);

                            List<Line> lineList = new List<Line>();
                            for (int i = 0; i < round; i++)
                            {
                                double value = (length * 2) - (maxHeight * (i + 1));
                                XYZ endpoint = crossPoints.Key + sideLineH.Direction.Multiply(value);
                                KeyValuePair<XYZ, XYZ> crossPoints4 = Utility.CrossProduct(elements[0] as Conduit, endpoint, 30);
                                XYZ intersection = Utility.FindIntersectionPoint(crossPoints3.Key, crossPoints3.Value, crossPoints4.Key, crossPoints4.Value);
                                crossPoints4 = Utility.CrossProduct(Utility.GetLineFromConduit(elements[0] as Conduit), intersection, 10);

                                obj = new LineCollection();
                                obj.line = Line.CreateBound(crossPoints4.Key, crossPoints4.Value);
                                verticalLineList.Add(obj);
                            }
                            if (((length * 2) / maxHeight) <= round)
                            {
                                obj = new LineCollection();
                                obj.line = sideLineV;
                                verticalLineList.Add(obj);
                            ...
                            round = Math.Floor((sideLineV.Length) / maxWidth);
                            for (int i = 0; i < round; i++)
                            {
                                double value = sideLineV.Length - (maxWidth * (i + 1));
                            ...
                            if ((sideLineV.Length) / maxWidth <= round)
                            {
                                obj = new LineCollection();
                                obj.line = sideLineH;
                                horizontalLineList.Add(obj);
